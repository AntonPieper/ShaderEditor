...
order: 2
name: "Sensorium"
desc: "A raymarched shrine driven by device orientation, motion sensors, magnetic field, light, pressure, proximity, battery, wallpaper offset, and time."
code: |-
...
---
This is still real ray marching, but the sky is now deliberately robust. Even before any geometry is hit, the shader builds a strong UV-driven atmosphere from `date`, `daytime`, `light`, `magnetic`, and `gyroscope`, so the sample never collapses into a flat screen on devices with different sensor defaults.

The 3D scene itself is a shrine made from signed-distance primitives: a central tower (`sdBox()`), an orbital ring (`sdTorus()`), a glowing core (`sdSphere()`), and a floating orb. `scene()` warps those shapes with `orientation`, `rotationVector`, `rotationMatrix`, `inclination`, and `inclinationMatrix`, while `gravity`, `gyroscope`, and `magnetic` add smaller secondary rotations and sky motion.

System uniforms each steer a specific part of the composition. `battery` stretches the tower. `powerConnected` changes the accent color. `notificationCount` and `lastNotificationTime` pulse the ring. `pressure` changes fog density. `proximity` expands the core. `offset` shifts and rotates the shrine itself, which keeps the sample centered in the editor preview while still responding to launcher page movement in wallpaper mode.

`safeMat()` matters because sensor matrices may be absent or zeroed on some devices. Rather than trusting them blindly, the shader blends them against identity so the sample still renders everywhere.

**Uniforms used:** `time`, `ftime`, `battery`, `powerConnected`, `nightMode`, `notificationCount`, `lastNotificationTime`, `date`, `daytime`, `offset`, `gravity`, `gyroscope`, `magnetic`, `orientation`, `inclination`, `inclinationMatrix`, `rotationMatrix`, `rotationVector`, `light`, `pressure`, `proximity`, `resolution`.

**Try this:** Lower march steps from `96` to `64` for weaker phones, or boost the `battery` multiplier in tower height if you want charge level to dominate the silhouette more strongly.
