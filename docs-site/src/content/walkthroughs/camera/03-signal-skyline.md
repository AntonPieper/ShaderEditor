...
order: 3
name: "Signal Skyline"
desc: "An audio-reactive live wallpaper that reads time, battery, volume, microphone, wallpaper offset, and notifications."
code: |-
...
---
This shader treats the phone like a live status feed. `micAmplitude` and `mediaVolume` drive the skyline energy, `battery` raises and lowers the horizon, `powerConnected` swaps accents into a charging palette, and `notificationCount` adds extra beacons. If a notification just arrived, `lastNotificationTime` is small and a pulse ring blooms outward; if no notification has ever arrived, ShaderEditor returns `NaN`, so the shader guards it before using it.

The key detail is the hash function. The skyline uses `hash21(vec2)` instead of a one-float hash, which keeps stars, windows, and building variation stable on mobile GPUs. That matters for live wallpapers because low-precision or correlated hashes quickly turn into visible vertical banding.

Time comes from two different uniforms for different jobs. `date` provides year, month, day, and seconds-of-day, while `daytime` is already split into hour, minute, second. One Android quirk: `date.y` comes straight from `Calendar.MONTH`, so January is `0`, not `1`.

`offset` is what makes it feel like a real wallpaper instead of a full-screen demo. On the home screen, horizontal paging shifts `wrapX`, so the skyline pans with launcher pages. `second` and `subsecond` keep window blinking and beat timing stable without leaning too hard on ever-growing `time`.

**Uniforms used:** `time`, `second`, `subsecond`, `date`, `daytime`, `battery`, `powerConnected`, `nightMode`, `notificationCount`, `lastNotificationTime`, `offset`, `mediaVolume`, `micAmplitude`, `resolution`.

**Try this:** Lower `cols` for chunkier retro buildings, or bias `audio` toward `micAmplitude` if you want a voice-reactive wallpaper instead of a media-volume one.
